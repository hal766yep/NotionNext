...
export default async function getPageProperties(
  ...
  // 优先使用 featured_image (在 ext 字段中), 否则使用 Notion 页面封面
  const featuredImage = properties.ext?.featured_image
  if (featuredImage) {
    properties.pageCover = mapImgUrl(featuredImage, value) ?? ''
    properties.pageCoverThumbnail = mapImgUrl(featuredImage, value, 'block') ?? ''
  } else {
